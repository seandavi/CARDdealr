.catalog_file=system.file('data_catalog/catalog.csv',package='CARDdealr')
.dataset_details_file = system.file('data_catalog/dataset_details.yaml',
                                    package='CARDdealr')
#.dataset_details_file = 'abc.yaml'
...
available_datasets <- function() {
  ...
  class(res) = c('cdlr_avail_datasets',class(res))
  res
}
...
dataset_details = function() {
  if(!file.exists(.dataset_details_file)) {
    return(list(datasets=list(), eval_date=as.character(Sys.Date())))
  }
  yaml::yaml.load_file(.dataset_details_file)
}

write_dataset_details = function(dd) {
  dd[['eval_date']] = as.character(Sys.Date())
  writeLines(yaml::as.yaml(dd),'inst/data_catalog/dataset_details.yaml')
}
...
create_dataset_details = function() {
  ...
  res = lapply(unique(z$accessor), add_or_update_dataset_details)
}
